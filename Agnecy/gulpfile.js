...
gulp.task('serve', function() {
   browserSync.init({server: paths.app.html.dest, notify: false});
});

/* Таск для работы Browsersync, автообновление браузера: */
gulp.task('watch', ['serve'], function() {
   gulp.watch(paths.watch.grid, ['grid']);
   gulp.watch(paths.watch.html, ['html']);
   gulp.watch(paths.watch.css, ['cssCommon']);
   gulp.watch(paths.watch.js, ['jsCommon']);
...
gulp.task('fonts', ['fontsEot', 'fontsWoff', 'fontsWoff2', 'fontsSvg']);
...
/* Таск для production: */
gulp.task('public', ['clean', 'img', 'fonts', 'dist']);
